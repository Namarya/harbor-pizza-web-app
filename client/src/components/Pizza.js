    ...
    >
      <div className="container" onClick={handleShow}>
        ...
        <img
          ...
          alt=""
          className="img-fluid"
          style={{ width: "20rem", height: "15rem", objectFit: "cover", cursor:"pointer" }}
        />
      </div>
      ...
      <div className="flex-container m-1">
          ...
          <div className="w-100 m-1">
            ...
            >
              {pizza.sizes.map((size) => {
                return <option key={pizza._id + JSON.stringify(size)} value={size}>{size}</option>;
              })}
            </select>
          </div>
        )}
        {pizza.sizes.length > 1 ? (
          ...
          >
            {[...Array(10).keys()].map((x, i) => {
              return <option key={i * 10} value={i + 1}>{i + 1}</option>;
            })}
          </select>
        </div>
        )}
